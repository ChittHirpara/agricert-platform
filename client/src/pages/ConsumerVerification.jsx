                    ...
                    <div className="animate-fadeIn space-y-6">
                        ...
                        <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl p-6 text-white shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
                            ...
                            <div className="bg-black/30 rounded-lg p-3 w-full sm:w-auto overflow-hidden">
                                ...
                                <span className="font-mono text-xs text-green-400 break-all">{result.blockchain?.txHash}</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <span className="text-gray-900 text-lg">{result.product?.cropName || "Unknown"}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <div>
                                            <span className="block text-xs text-gray-500 uppercase font-semibold">Quantity</span>
                                            <span className="text-gray-900">{result.product?.quantity || "N/A"}</span>
                                        </div>
                                        <div>
                                ...
                                {result.farmer ? (
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <span className="text-gray-900 text-lg">{result.farmer?.name || "Unknown Farm"}</span>
                                        </div>
                                        <div>
                                            <span className="block text-xs text-gray-500 uppercase font-semibold">Location</span>
                                            <span className="text-gray-900 flex items-center gap-1">
                                                <MapPin size={16} className="text-red-500" /> {result.product?.location || "N/A"}
                                            </span>
                                        </div>
                                    </div>
                                ) : (
                                ...
                                {result.certification ? (
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="bg-green-50 rounded-lg p-4">
                                            ...
                                            <span className="text-green-600 font-bold flex items-center gap-1"><CheckCircle2 size={16} /> {result.certification?.status || "CERTIFIED"}</span>
                                        </div>
                            ...
                            <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm md:col-span-2 mt-2">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="absolute top-1/2 left-4 h-1 bg-green-500 transform -translate-y-1/2 z-0 hidden sm:block" style={{ width: result.product?.status === 'certified' ? '100%' : '80%' }}></div>
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between relative z-10 gap-6 sm:gap-0">
                                        ...
                                        <div className="flex flex-row sm:flex-col items-center gap-4 sm:gap-2">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-md ${result.auction ? 'bg-green-500 text-white shadow-green-200' : 'bg-gray-100 text-gray-400 border-2 border-dashed border-gray-300'}`}>
                                                <Calendar size={20} />
                                            </div>
                                            <div className="text-left sm:text-center">
                                                <span className={`block font-bold text-sm ${result.auction ? 'text-gray-900' : 'text-gray-400'}`}>Auction Ready</span>
